...
challenge/
  ...
  results/
    baseline/
      ...
      baseline.json ← json file required for leaderboard submission (generated by test.py)
```
...
This prints weighted F1 and per-class F1 to stdout, logs a confusion matrix to wandb, and writes `Your_Team_Name.json` to `results/<team_name>/`.

### 5. Submit to the leaderboard

Copy your JSON file to the leaderboard repository and push it:

```bash
cp results/Your_Team_Name/Your_Team_Name.json /path/to/leaderboard-repo/submissions/
cd /path/to/leaderboard-repo
git add submissions/Your_Team_Name.json
git commit -m "Submit Your Team Name"
git push
```

The leaderboard updates automatically within seconds. View rankings at:
**[Leaderboard repo link — TO BE ANNOUNCED]**
